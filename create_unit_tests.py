...
def create_tests(jar_path):
    ...
    classes["talon"].append(
        "com/ctre/phoenix/motorcontrol/can/BaseMotorController.class")
        ...
        if len(matches) != 0:
            ...
            if len(enumerations) == 0:
                ...
                tests += "    for({0} {1} : {0}.values())".format(
                    enumerations[0], prepare_variable_name(
                        enumerations[0])) + "\n    {\n"
                tests += "        %s.%s(%s);\n" % (objName, func,
                                                   ", ".join(arg_replacement))
                tests += "    }\n"
...
create_tests(
    r'C:\Users\PJ\.gradle\caches\modules-2\files-2.1\{artifact_package}\{artifact_name}\{lib_version}\{lib_hash}/{artifact_name}-{lib_version}.jar'
    .format(lib_hash=LIB_HASH,
